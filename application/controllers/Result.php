...
class Result extends CI_Controller {
	...
	public function index($limit='0',$status='0')
	{
		...
		// fetching exam list
		$data['exam_list']=$this->result_model->exam_list();
		// group list
	...
	function generate_report(){
		...
		$csvdata=$this->lang->line('result_id').",".$this->lang->line('email').",".$this->lang->line('first_name').",".$this->lang->line('last_name').",".$this->lang->line('group_name').",".$this->lang->line('exam_name').",".$this->lang->line('score_obtained').",".$this->lang->line('percentage_obtained').",".$this->lang->line('status')."\r\n";
		foreach($result as $rk => $val){
		$csvdata.=$val['rid'].",".$val['email'].",".$val['first_name'].",".$val['last_name'].",".$val['group_name'].",".$val['exam_name'].",".$val['score_obtained'].",".$val['percentage_obtained'].",".$val['result_status']."\r\n";
		}
		$filename=time().'.csv';
	...
	function view_result($rid){
		...
		$data['questions']=$this->exam_model->get_questions($data['result']['r_qids']);
		$data['options']=$this->exam_model->get_options($data['result']['r_qids']);

		}
		// top 10 results of selected exam
	$last_ten_result = $this->result_model->last_ten_result($data['result']['quid']);
	$value=array();
     $value[]=array('Exam Name','Percentage (%)');
	...
	$data['result']=$this->exam_model->get_exam($quid);
	if($data['result']['gen_certificate']=='0'){
		exit();
	}
		// save qr 
