...
Class Qbank_model extends CI_Model
{
  ...
  function question_list($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 ...
		 $this->db->or_where('mock_qbank.qid',$search);
		 $this->db->or_like('mock_qbank.question',$search);
		 $this->db->or_like('mock_qbank.description',$search);
	 ...
	 if($lid!='0'){
		 ...
		 $this->db->join('mock_category','mock_category.cid=mock_qbank.cid');
	 $this->db->join('mock_level','mock_level.lid=mock_qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
		$this->db->order_by('mock_qbank.qid','desc');
		$query=$this->db->get('mock_qbank');
 ...
 function get_option($qid){
	 $this->db->where('qid',$qid);
	 $query=$this->db->get('mock_options');
	 return $query->result_array();
 ...
 function remove_question($qid){
	 ...
	 $this->db->where('qid',$qid);
	 if($this->db->delete('mock_qbank')){
		  $this->db->where('qid',$qid);
			$this->db->delete('mock_options');
		 return true;
	 }else{
 ...
 function insert_question_1(){
	 ...
	 $this->db->insert('mock_qbank',$userdata);
	...
	$userdata=array(
	 ...
	 $this->db->insert('mock_options',$userdata);	 
 ...
 function insert_question_5(){
  ...
  function update_question_1($qid){
	 ...
	 $this->db->where('qid',$qid);
	 $this->db->update('mock_qbank',$userdata);
	 $this->db->where('qid',$qid);
	$this->db->delete('mock_options');
	 foreach($this->input->post('option') as $key => $val){
 ...
 function category_list(){
	 ...
	 $query=$this->db->get('mock_category');
	 return $query->result_array();
 ...
 function update_category($cid){
		...
		if($this->db->update('mock_category',$userdata)){
 ...
 function remove_category($cid){
	 ...
	 $this->db->where('cid',$cid);
	 if($this->db->delete('mock_category')){
		 return true;
	 }else{
 ...
 function insert_category(){
		...
		if($this->db->insert('mock_category',$userdata)){
...
// level function start
 ...
 function update_level($lid){
		...
		if($this->db->update('mock_level',$userdata)){
 ...
 function insert_level(){
		...
		if($this->db->insert('mock_level',$userdata)){
...
$question_type=$this->lang->line('long_answer');	
	...
	if($this->db->insert('mock_qbank',$insert_data)){
		$qid=$this->db->insert_id();
		$optionkeycounter = 4;
		if($ques_type=="0" || $ques_type==""){
		for($i=1;$i<=10;$i++){
			if($singlequestion[$optionkeycounter] != ""){
				...
				$this->db->insert("mock_options",$insert_options);
				$optionkeycounter++;
				}
