...
Class User_model extends CI_Model
{
 function login($username, $password)
 {
   ...
   $this -> db -> where('mock_admin_users.password', MD5($password));
   }
    $this -> db -> where('mock_admin_users.email', $username);
    $this -> db -> where('mock_admin_users.verify_code', '0');
    $this -> db -> join('mock_group', 'mock_admin_users.gid=mock_group.gid');
  $this->db->limit(1);
    $query = $this -> db -> get('mock_admin_users');
 ...
 function user_list($limit){
	 if($this->input->post('search')){
		...
		$this->db->order_by('mock_users.uid','desc');
		 $this -> db -> join('mock_group', 'mock_users.gid=mock_group.gid');
		 $query=$this->db->get('mock_users');
 ...
 function verify_code($vcode){
	...
	$query=$this->db->get('mock_admin_users');
		if($query->num_rows()=='1'){
			...
			$this->db->update('mock_admin_users',$userdata);
			return true;
		}else{
 ...
 function insert_user(){
		...
		if($this->db->insert('mock_users',$userdata)){
...
$queryr=$this->db->get('mock_admin_users');
...
$new_password=rand('1111','9999');
 ...
 if($this->config->item('protocol')=="smtp"){
			...
			$this->db->where('email', $toemail);
 			$this->db->update('mock_admin_users',$user_detail);
			return true;
			}
 ...
 function update_user($uid){
	 $logged_in=$this->session->userdata('logged_in');
		...
		if($this->input->post('password')!=""){
		 ...
		 $this->db->where('uid',$uid);
		if($this->db->update('mock_users',$userdata)){
 ...
 function update_group($gid){
		...
		if($this->db->update('mock_group',$userdata)){
 ...
 function get_user($uid){
	...
	$this->db->where('mock_users.uid',$uid);
	   $this -> db -> join('mock_group', 'mock_users.gid=mock_group.gid');
$query=$this->db->get('mock_users');
	 return $query->row_array();
 ...
 function insert_group(){
		...
		if($this->db->insert('mock_group',$userdata)){
